...
const MyInvoices = ({ navigation }) => {
    ...
    const getInvoices = useCallback((userId) => {
        ...
        onValue(invoicesRef, (snapshot) => {
            ...
            if (data) {
                ...
                console.log("array inv", invoicesArray);
    ...
    const getActiveServices = useCallback((userId) => {
        setLoading(true);
                    ...
                    {(loading || payLoading) && !refreshing ? (
                        ...
                        <View style={styles.invoiceList}>
                            {invoices.map((item, index) => {
                                // Extract the nested invoice object if it exists
                                ...
                                const invoiceAmount = invoice.invoice_amount || invoice.invoice_total || 0;
                                    ...
                                    <View key={index} style={styles.card}>
                                        ...
                                        <View style={styles.accountContainer}>
                                            <View style={styles.accountInfo}>
                                                <Text style={styles.accountName}>{invoiceNumber}</Text>
                                                <Text style={styles.accountNumber}>Tanggal: {invoiceDate}</Text>
                                            </View>
                                        </View>
                                        ...
                                        {details.length > 0 && (
                                            <View style={styles.detailsList}>
                                                {details.map((detail, dIndex) => (
                                                    <Text key={dIndex} style={styles.detailText}>• {detail}</Text>
                                                ))}
                                            </View>
                                        )}
                                        ...
                                        <View style={styles.serviceDetails}>
                                            <View style={styles.detailItem}>
                                                ...
                                                <Text style={styles.detailValue}>Rp {formatNumberWithCommas(invoiceAmount)}</Text>
                                            </View>
                                        </View>
                        ...
                        <View style={[styles.card, styles.noDataCard]}>
                            {activeServices.length > 0 ? (
                                ...
                                <>
                                    <View style={[styles.noDataIcon, { backgroundColor: 'rgba(255, 159, 10, 0.1)' }]}>
                                        <Ionicons name="alert-circle-outline" size={60} color="#FF9F0A" />
                                    </View>
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <Text style={styles.payBtnText}>Hubungkan Layanan</Text>
                                        </LinearGradient>
                                    </TouchableOpacity>
                                </>
                            )}
